<span data-filter="chapter" class="filter-container relative w-full md:w-fit">
    <button class="filter-trigger relative w-full flex gap-2 items-center text-[#FFD700] border-[2px] border-[$FFD700] rounded text-xs px-4 py-1 cursor-pointer">
    ...
    <div class="filter-popup min-chapter-form absolute w-full md:w-[20vw]  lg:w-[17vw] bottom-full md:bottom-auto md:top-full left-0 my-2 rounded bg-[#252525] z-50 hidden flex filter-chapter py-2 px-4">
        <form class="flex flex-col w-full gap-2 items-center text-white">
            @csrf
            <input name="min-chapter" min="0" type="number" placeholder="{{ __('browse.example_chapter') }}" class="min-chapter-input w-full text-xs p-1">
